...
특히 서비스별 DB가 분리되는 MSA 환경에서는 분산 트랜잭션으로 인한 데이터 정합성 이슈가 발생하므로, 이를 해결하기 위한 Saga 패턴(보상 트랜잭션) 전략이 필요하다. 중앙 오케스트레이터 없이 각 서비스가 이벤트를 구독/발행하며 흐름을 이어가는 Choreography 방식으로 설계했다.
...
### 전체 흐름 요약
1. **주문 접수**
   사용자·상품·쿠폰 조회 → 주문 저장(CREATED) → 200 OK
   ※ 조회 실패면 예외 → 에러 응답 (재고 부족은 여기서 안 걸림)
   사용자 화면: "주문 처리 중..."

2. **커밋 후 이벤트 발행**
   OrderEvent.Created → 리스너 3개가 각자 다른 스레드로 출발
   ※ 여기서부터 Saga 시작 (롤백 불가, 되돌리려면 보상)

3. **잔액·쿠폰·재고 차감 (병렬)**
   각자 끝나면 Redis 체크리스트에 SUCCESS / FAILED 기록
   기록할 때마다 "다 찼나?" 확인 → 마지막 하나만 다음으로
   ※ 분산 락으로 "확인 + 발행" 구간 직렬화 (동시성 제어)
   → 전부 성공: OrderEvent.PaymentWaited
   → 하나라도 실패: OrderEvent.Failed (집계 상태 동봉)

4. **결제**
   OrderEvent.PaymentWaited → 결제 수행 → PaymentEvent.Paid
   ※ 실패 시 PayFailed / Canceled → OrderEvent.Failed 로 변환 (보상 경로 통합)

5. **주문 확정**
   주문서비스가 수신 → CREATED에서 COMPLETED로 → OrderEvent.Completed 발행
   ※ 실패 시 CompleteFailed → 결제 자동 취소 → OrderEvent.Failed(보상)로 통합

6. **뒷정리 (병렬, 서로 무관)**
   랭킹 집계 / 외부 플랫폼 전송
   ※ 실패해도 보상 없음
   ※ 화면 갱신은 이 흐름 밖. 폴링·SSE·푸시로 별도 처리

---

**보상 트랜잭션 (OrderEvent.Failed 수신 시)**
각 리스너가 자기 단계가 SUCCESS였을 때만 실행
· 잔액 환불 / 쿠폰 사용취소 / 재고 복구 / 결제 취소
→ 주문 CANCELLED
