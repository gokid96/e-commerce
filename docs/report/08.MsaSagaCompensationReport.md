...
이벤트 발행과 로컬 트랜잭션의 원자성을 보장하기 위해 Outbox 패턴을 적용했다. 도메인 로직과 같은 트랜잭션 안에서 아웃박스 레코드를 저장(`@TransactionalEventListener(BEFORE_COMMIT)`)하고, 커밋 후 별도로 Kafka에 발행한다. 발행된 이벤트는 각 서비스의 Outbox 컨슈머가 되받아 해당 레코드를 삭제하며, 삭제되지 않고 남은 레코드는 스케줄러가 재전송하도록 설계했다.

Outbox 컨슈머 그룹은 `spring.application.name`을 부여해 서비스별로 분리한다. 하나의 그룹 ID를 여러 서비스가 공유하면 Kafka가 파티션을 나눠 할당하므로, A 서비스가 발행한 이벤트의 확인 메시지를 B 서비스가 가져가 자기 DB에서 삭제를 시도하게 된다. 삭제 대상이 없어 no-op으로 끝나고, A의 레코드는 영원히 남아 재전송이 반복된다.

> 재전송 스케줄러는 현재 product 서비스에만 활성화되어 있다(`@EnableScheduling`). order·payment에서의 활성화는 컨슈머 멱등성 확보와 함께 진행한다 — 아래 한계 참조.
...
설계대로 동작하는지 실패 경로를 역추적해 검증했고, 다음 세 지점에서 구현이 설계를 따라가지 못하고 있음을 확인해 수정했다.

**1. 보상 이벤트가 롤백과 함께 유실되던 문제**

실패 이벤트를 실패하는 트랜잭션 안에서 발행하고 예외를 던지는 구조였다. 아웃박스 레코드는 `BEFORE_COMMIT` 시점에 저장되므로, 롤백되면 이벤트도 함께 사라진다. 즉 보상이 가장 필요한 순간에 보상 트리거가 발행되지 않았다.

실패 이벤트 발행을 별도 빈으로 분리하고 `REQUIRES_NEW`를 적용해, 원 트랜잭션의 성패와 무관하게 커밋되도록 했다. 같은 클래스 내 호출은 프록시를 거치지 않아 전파 속성이 무시되므로 빈 분리가 필수다.

**2. Outbox 컨슈머 그룹이 서비스 간 공유되던 문제**

`common:outbox` 모듈의 컨슈머가 고정 그룹 ID를 쓰고 있어, 모듈을 포함한 세 서비스가 하나의 그룹을 공유했다. 확인 메시지가 엉뚱한 서비스로 전달되어 레코드가 삭제되지 않는 구조였다. 그룹 ID에 `spring.application.name`을 부여해 분리했다.

**3. DLT(Dead Letter Topic) 미구성**

컨슈머 예외 시 ack가 수행되지 않아 동일 메시지를 무한 재소비했다. 재시도를 3회로 제한하고 한계를 넘긴 메시지는 `<원본토픽>.DLT`로 격리하도록 `common:message`에 공통 에러 핸들러를 추가했다.

---

## 남은 한계

- 멱등성 미구현 — 동일 이벤트가 중복 소비되면(Kafka at-least-once) 중복 결제·중복 재고 복구가 발생할 수 있다. `Payment.orderId` 유니크 제약과 컨슈머 측 멱등 처리, 주문 상태 가드가 필요하다. 재전송 스케줄러를 order·payment에서 활성화하려면 이 작업이 선행되어야 한다.
...
- 원격 호출의 트랜잭션 경계 — 잔액·쿠폰 차감이 결제 트랜잭션 안에서 이뤄져, HTTP 응답 시간만큼 DB 커넥션을 점유한다. 오케스트레이션을 트랜잭션 밖으로 분리해야 한다(쿠폰 서비스는 Facade가 원격 호출을 담당하는 구조로 이미 분리되어 있다).
...
다만 이벤트 기반 MSA는 시스템 복잡도를 크게 높인다. 설계도만으로는 정합성이 확보되지 않으며, 실패 경로를 실제로 역추적해봐야 구현이 설계를 따라가는지 알 수 있다. 위 세 건이 그 과정에서 드러난 것들이고, 남은 한계 또한 같은 방식으로 해소해야 운영 수준의 신뢰성에 도달한다. 아키텍처 도입 여부와 정교함의 수준은 서비스 규모와 운영 환경을 고려해 단계적으로 결정하는 것이 바람직하다.
